...
const Index = () => {
  ...
  const isMobile = useIsMobile(); // Usa o hook para detectar mobile
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // --- Ajustes no Efeito de Zoom ---
      // Parâmetros diferentes para mobile e desktop
      const scaleFactor = isMobile ? 2500 : 5000; // Zoom mais rápido no mobile
      const maxScale = isMobile ? 1.35 : 1.25; // Zoom máximo maior no mobile
      const baseScale = 1.1;

      // Cálculo da nova escala
      // Usar uma função seno para criar um efeito de "respiração" mais suave
      // e garantir que volte à escala base eventualmente.
      // O valor 5000 aqui controla a "distância" do scroll para completar um ciclo de zoom in/out.
      const scrollProgress = scrollY / 5000; // Ajuste este valor para controlar a velocidade do ciclo
      const scaleOffset =
        (Math.sin(scrollProgress * Math.PI - Math.PI / 2) + 1) / 2; // Varia de 0 a 1
      const targetScale = baseScale + (maxScale - baseScale) * scaleOffset;

      // Aplica a escala (não precisa mais do Math.min por causa do seno)
  ...
  return (
    <div className="min-h-screen flex flex-col page-wrapper relative">
      ...
      <div className="fixed inset-0 z-[-2] overflow-hidden">
        ...
        <div
          ...
          style={{
            ...
            transition: "transform 0.25s ease-out", // Transição ligeiramente mais rápida
            opacity: 0.4,
            ...
            <div className="flex flex-col">
              ...
              >
                <TypeAnimationEnhanced
                  ...
                  repeat={1}
                  cursor={true}
                  className="text-5xl md:text-5xl lg:text-6xl font-medium text-balance leading-tight break-words text-white"
                />
              </motion.div>
      ...
      {/* Carrossel de Projetos Destacados */}
      ...
      <section className="py-24 relative">
        <div className="container px-6 max-w-7xl mx-auto">
          ...
          {/* Restaurando o grid de serviços */}
              ...
              <ScrollAnimator key={index} delay={index * 100}>
                <div className="flex flex-col h-full p-8 rounded-xl border border-gray-100/10 dark:border-gray-800/30 bg-gray-900/40 backdrop-blur-sm hover:border-emerald-800/40 dark:hover:border-emerald-800/40 transition-all">
                  ...
                  <div className="mt-auto">
                    <Link
                      to="/contact" // Mantendo link original, pode ser ajustado se necessário
      ...
      <section id="contact" className="py-20 relative overflow-hidden">
        ...
        <div className="container px-6 md:px-6 relative z-10">
          ...
          {/* Botão único CTA para WhatsApp */}
          <div className="text-center mt-8 mb-16">
            {" "}
            {/* Adicionado margem inferior */}
            {/* Substitua SEUNUMERO pelo número de telefone correto e TEXTO pela mensagem inicial */}
            <Button
              asChild
              size="lg"
            ...
            >
              <a
                href="https://wa.me/SEUNUMERO?text=Olá!%20Gostaria%20de%20conversar%20sobre%20um%20projeto."
                target="_blank"
                rel="noopener noreferrer"
              >
                {/* Idealmente usar um ícone de WhatsApp aqui, usando MessageCircle como fallback */}
