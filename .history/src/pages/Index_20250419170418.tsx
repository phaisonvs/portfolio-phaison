...
const Index = () => {
  ...
  useEffect(() => {
    ...
    // Definir as configurações do efeito com valores mais sutis
    const config = {
      baseScale: 1.05, // Escala base da imagem (reduzida para ser mais sutil)
      scaleRange: 0.03, // Quanto a escala pode aumentar/diminuir (reduzido)
      maxScroll: 2000, // Ponto máximo de scroll para efeito completo
      bounceTime: 8, // Tempo em segundos para um ciclo completo de bounce (mais lento)
      verticalSpeed: 0.05, // Velocidade do movimento vertical (reduzida)
      bounceAmount: 15, // Amplitude do efeito de bounce (reduzida)
    };

    function applyBounceEffect() {
      const currentTime = Date.now() / 1000; // Tempo atual em segundos
      const bounceValue = Math.sin((currentTime * Math.PI) / config.bounceTime); // Cálculo do bounce com velocidade reduzida
      return bounceValue * config.bounceAmount; // Retorna um valor de deslocamento mais sutil
    }
    ...
    const handleScroll = () => {
      ...
      // Calcular o deslocamento vertical (suavizado)
      ...
      // Calcular a escala com base no progresso do scroll (mais sutil)
      const zoomFactor =
        config.baseScale +
        config.scaleRange * Math.sin(scrollProgress * Math.PI * 0.5);

      // Aplicar transformação combinando movimento vertical e zoom suave
      backgroundImage.style.transform = `translate3d(0, ${
        bounceOffset - verticalOffset
      }px, 0) scale(${zoomFactor})`;
    };

    // Executar o efeito uma vez para definir a posição inicial
    handleScroll();

    // Setup para o efeito de bounce contínuo e suave
    const bounceInterval = setInterval(() => {
      if (!backgroundImage) return;
      requestAnimationFrame(handleScroll);
    }, 33); // Aproximadamente 30fps para economia de recursos
            ...
            <div className="flex flex-col">
              ...
              >
                <TypeAnimationEnhanced
                  sequence={["Veja como construí soluções usando porqu"]}
                  wrapper="h1"
                  speed={30}
                  deletionSpeed={30}
