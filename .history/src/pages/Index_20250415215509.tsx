...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const config = {
      ...
      verticalSpeed: 0.2, // Velocidade do movimento vertical
      maxScroll: 1500, // Ponto máximo de scroll para efeito completo
      bounceTime: 8000, // Tempo em ms para completar um ciclo de bounce
    };

    // Função para aplicar efeito de bounce infinito
    const applyBounceEffect = () => {
      const now = Date.now();
      // Cria um valor sinusoidal entre 0 e 1 baseado no tempo atual
      const bounceFactor =
        ((Math.sin((now / config.bounceTime) * Math.PI * 2) + 1) / 2) * 0.03;
      return bounceFactor;
    };
