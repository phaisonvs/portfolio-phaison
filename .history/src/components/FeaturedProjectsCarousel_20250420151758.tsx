...
export function FeaturedProjectsCarousel({
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <CarouselContent className="-ml-0 xs:-ml-4">
          {projects.map((project) => (
            <CarouselItem
              key={project.id}
              className="min-w-0 pl-2 xs:pl-4 basis-full sm:basis-1/2 lg:basis-1/3"
            >
              {/* ... Card Content ... */}
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                ...
                className="group relative overflow-hidden rounded-xl h-full flex flex-col bg-gray-900/40 backdrop-blur-sm border border-gray-800/30 hover:border-emerald-800/40 transition-all duration-300 mx-auto w-full max-w-[450px] sm:max-w-none"
              >
        ...
        {/* ... Navigation Buttons ... */}
        <div className="flex items-center justify-center sm:justify-end gap-1 sm:gap-2 mt-4 sm:mt-6">
          <CarouselPrevious className="static translate-y-0 h-8 w-8 sm:h-9 sm:w-9 mr-1 sm:mr-2 border-gray-700 bg-gray-800 hover:bg-gray-700 text-white" />
          <CarouselNext className="static translate-y-0 h-8 w-8 sm:h-9 sm:w-9 border-gray-700 bg-gray-800 hover:bg-gray-700 text-white" />
        </div>
      </Carousel>
    </div>
  );
