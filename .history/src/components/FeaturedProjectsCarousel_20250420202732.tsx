...
const variants = {
  enter: (direction: number) => ({
    x: direction > 0 ? "100%" : "-100%", // Entra deslizando da direita ou esquerda
    opacity: 0,
    position: "absolute", // Necessário para AnimatePresence
  }),
  center: {
    zIndex: 1,
    x: 0,
    opacity: 1,
    position: "relative", // Ou 'static' se não precisar de zIndex alto
  },
  exit: (direction: number) => ({
    zIndex: 0,
    x: direction < 0 ? "100%" : "-100%", // Sai deslizando para a direita ou esquerda
    opacity: 0,
    position: "absolute",
  }),
};
...
export function FeaturedProjectsCarousel({
  ...
  // Efeito para ajustar a altura do container baseado no conteúdo atual
  // (Isso pode ser complexo e pode precisar de ajustes finos)
  // useEffect(() => {
  //   if (carouselRef.current) {
  //     // Tenta pegar a altura do primeiro card visível como referência
  //     const firstChild = carouselRef.current.querySelector(':scope > div > div');
  //     if (firstChild) {
  //         setContainerHeight(firstChild.clientHeight);
  //     }
  //   }
  // }, [currentProjects]); // Recalcula quando os projetos mudam
    ...
    <div className="space-y-8 w-full max-w-full overflow-x-hidden">
      ...
      <div className="w-full max-w-full px-0 overflow-x-hidden">
        <div className="relative w-full">
          ...
          <div
            ref={carouselRef}
            className="relative flex w-full overflow-hidden min-h-[450px]" // Adicionado min-h-[450px]
            style={{ height: containerHeight }} // Usa estado para altura, ou 'auto'/'min-h' podem bastar
          ...
          >
            ...
            <AnimatePresence initial={false} custom={direction} mode="sync">
              {currentProjects.map((project, index) => (
                <motion.div
                  key={page * visibleCards + index} // Key precisa mudar com a página E o card
                  className="shrink-0 grow-0 basis-auto px-2 absolute top-0 left-0 w-full h-full"
                  style={{
                  ...
                  // Remover drag temporariamente para simplificar
                  // drag="x"
