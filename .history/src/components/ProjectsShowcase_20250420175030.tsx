...
export function ProjectsShowcase({ projects }: ProjectsShowcaseProps) {
  ...
  // Definir opções com tipo explícito para evitar erro de tipo
  const emblaOptions: EmblaOptionsType = {
    loop: true,
    align: "start",
    slidesToScroll: 1,
    // A opção draggable é geralmente padrão ou controlada pelo CSS touch-action
    // Não adicionaremos explicitamente aqui para evitar o erro de tipo,
    // mas o comportamento de swipe deve funcionar por padrão.
  };
  ...
  useEffect(() => {
    ...
    // Chamar onSelect para definir o estado inicial corretamente
    onSelect();

    // Função de limpeza
    return () => {
      // Verificar se emblaApi ainda existe antes de chamar off
      // Isso evita erros se o componente for desmontado rapidamente
      if (emblaApi) {
        try {
          emblaApi.off("select", onSelect);
          emblaApi.off("reInit", onSelect);
        } catch (error) {
          // Opcional: Logar erro se a limpeza falhar
          // console.error("Failed to clean up Embla listeners:", error);
        }
      }
    };
    // Adicionar onSelect como dependência, pois é usado dentro do efeito
  }, [emblaApi, onSelect]);
