      ...
      >
        <CarouselContent className="-ml-0 sm:-ml-4">
          {projects.map((project, index) => (
            <CarouselItem
              key={project.id}
              className="min-w-0 pl-0 sm:pl-4 basis-full sm:basis-1/2 lg:basis-1/3"
            >
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.4, delay: index * 0.1 }}
                className="group relative overflow-hidden rounded-xl h-full flex flex-col bg-gray-900/40 backdrop-blur-sm border border-gray-800/30 hover:border-emerald-800/40 transition-all duration-300 mx-auto w-full max-w-md sm:max-w-none"
              >
