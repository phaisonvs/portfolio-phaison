...
export function ProjectsShowcase({ projects }: ProjectsShowcaseProps) {
  ...
  // Para controlar a direção da animação
  ...
  // Variantes de animação para o slider
  ...
  // Função atualizada para mudar o projeto com direção
  const paginate = (newDirection: number) => {
    const newIndex =
      (currentIndex + newDirection + projects.length) % projects.length;
      ...
      <div className="block sm:hidden mt-8">
        <div className="relative w-full max-w-[320px] min-w-[280px] mx-auto">
          ...
          <div className="absolute top-1/2 -left-4 transform -translate-y-1/2 z-10">
            <button
              onClick={() => paginate(-1)}
              className="bg-gray-800/70 backdrop-blur-sm p-2 rounded-full text-white hover:bg-gray-700/70 transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-opacity-50"
