...
export function FeaturedProjectsCarousel({
  ...
  return (
    <div className="space-y-8">
      ...
      <Carousel
        opts={{
          align: "start", // Começa pelo primeiro item
          loop: true,
          containScroll: "trimSnaps", // Evita que o último item fique cortado
        }}
        className="w-full" // Ocupa toda a largura
      >
        <CarouselContent className="-ml-4">
          {" "}
          {/* Margem negativa para compensar o padding do item */}
          {projects.map((project, index) => (
            <CarouselItem
              key={project.id}
              // Define a largura do item para diferentes breakpoints
              className="min-w-0 pl-4 basis-[90%] sm:basis-1/2 lg:basis-1/3"
            >
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
