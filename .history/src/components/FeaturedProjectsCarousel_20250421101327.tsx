    ...
    <div className="w-full">
      ...
      <div ref={carouselRef} className="relative w-full overflow-hidden">
        ...
        <ReactSpring.animated.div
          className="flex"
          style={{
            // Largura total baseada em visibleCards para permitir o scroll correto
            width: `${(projects.length / visibleCards) * 100}%`,
            x: x.to((val) => `${val}%`), // Animação percentual
          }}
        >
          {projects.map((project, index) => (
            ...
            <div
              key={project.id}
              className="px-2" // Espaçamento entre cards
              style={{
                width: `${100 / projects.length}%`, // Largura relativa ao container deslizante
                // Ou alternativamente, para garantir a largura baseada em visibleCards:
                // width: `${100 / visibleCards / (projects.length / visibleCards)}%` - simplifica para 100/projects.length
              }}
            >
              {/* Conteúdo do Card */}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={prevSlide}
            // Desabilitar o botão não é mais necessário com loop
