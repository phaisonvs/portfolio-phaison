...
export function FeaturedProjectsCarousel({
  ...
  useEffect(() => {
    // Calcula a largura de cada card em porcentagem em relação ao carrossel
  ...
  const nextSlide = () => {
    setCurrentIndex((prevIndex) => {
      if (prevIndex >= projects.length - 1) {
        // Se estiver no último projeto, volta para o início
        return 0;
      } else {
  ...
  const prevSlide = () => {
    setCurrentIndex((prevIndex) => {
      if (prevIndex <= 0) {
        // Se estiver no primeiro projeto, vai para o último
