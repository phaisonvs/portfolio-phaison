...
// Definições das variantes de animação para os cards
const variants = {
  enter: (direction: number) => ({
    x: direction > 0 ? 1000 : -1000, // Entra deslizando da direita (1000) ou esquerda (-1000)
    opacity: 0,
  }),
  center: {
    zIndex: 1,
  ...
  exit: (direction: number) => ({
    zIndex: 0,
    x: direction < 0 ? 1000 : -1000, // Sai deslizando para a direita (1000) ou esquerda (-1000)
    opacity: 0,
  }),
};
...
// Transição suave
const transition = {
  x: { type: "spring", stiffness: 300, damping: 30 }, // Ajuste para suavidade
...
export function FeaturedProjectsCarousel({
  ...
  // Mantém o índice da "página" atual de cards
  ...
  useEffect(() => {
    function handleResize() {
      ...
      // Reseta a página se o número de cards visíveis mudar para evitar estado inválido
      if (cards !== visibleCards) {
        setPage([0, 0]); // Reseta para a primeira página
      }
  ...
  }, [visibleCards]); // Adiciona visibleCards como dependência
  ...
  const paginate = (newDirection: number) => {
    // Calcula a nova página, fazendo o loop (0 a totalPages - 1)
    let newPage = (page + newDirection) % totalPages;
    if (newPage < 0) {
      newPage += totalPages; // Garante que o índice não seja negativo
    }
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "ArrowRight") {
        ...
        paginate(-1); // Retrocede
      }
    };
    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [page, totalPages]); // Adiciona dependências
  ...
  const handleTouchEnd = () => {
    ...
    if (Math.abs(difference) > 75) {
      // Verifica se o swipe foi significativo
      paginate(difference > 0 ? 1 : -1); // Avança se swipe para esquerda, retrocede se para direita
    }
  };
    ...
    <div className="space-y-8 w-full max-w-full overflow-x-hidden">
      ...
      <div className="w-full max-w-full px-0 overflow-x-hidden">
        <div className="relative w-full">
          {/* Wrapper para os cards com altura definida para AnimatePresence */}
          <div
            ref={carouselRef}
            className="relative flex w-full overflow-hidden"
            style={{ height: "auto" }} // Adapta altura ao conteúdo, pode precisar de ajuste
          ...
          >
            ...
            <AnimatePresence initial={false} custom={direction}>
              {currentProjects.map((project) => (
                <motion.div
                  key={project.id} // Usar ID do projeto como key
                  className="shrink-0 grow-0 basis-auto px-2 absolute top-0 left-0 w-full h-full" // Posicionamento absoluto para sobreposição
                  style={{
                    width: `${100 / visibleCards}%`, // Largura baseada nos cards visíveis
                    // Não precisamos mais de maxWidth aqui se o container pai controla
                    // O posicionamento será controlado pela animação
                  }}
                  custom={direction}
                  variants={variants}
                  initial="enter"
                  ...
                  drag="x" // Permitir arrastar (opcional, pode remover se não quiser)
                  dragConstraints={{ left: 0, right: 0 }}
                  dragElastic={1}
                  onDragEnd={(e, { offset, velocity }) => {
                    const swipe = Math.abs(offset.x) * velocity.x;
                    if (swipe < -10000) {
                      // Limiar para swipe para a esquerda
                      paginate(1);
                    } else if (swipe > 10000) {
                      // Limiar para swipe para a direita
                      paginate(-1);
                    }
                  }}
                >
