...
export function FeaturedProjectsCarousel({
  ...
  const prevSlide = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === 0
        ? Math.max(0, projects.length - visibleCards)
        : prevIndex - 1
    );
  };
  ...
  // Configura indicadores de página baseado no número total de slides
  const totalPages = Math.ceil((projects.length - visibleCards + 1) / 1);
  const currentPage = Math.floor(currentIndex / 1) + 1;
    ...
    <div className="space-y-8">
      ...
      >
        <div className="relative w-full">
          ...
          >
            {/* Container dos slides com transição suave */}
            <div
