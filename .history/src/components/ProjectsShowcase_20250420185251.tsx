...
export function ProjectsShowcase({ projects }: ProjectsShowcaseProps) {
  ...
  // Função genérica para navegação que funciona em ambos os modos
  const navigatePrev = useCallback(() => {
    // Para desktop usamos Embla
    if (window.innerWidth >= 640 && emblaApi) {
      emblaApi.scrollPrev();
      return;
    }

    // Para mobile usamos nossa própria lógica
    setDirection(-1);
  ...
  const navigateTo = useCallback(
    (index: number) => {
      ...
      // Para mobile, definimos a direção e o novo índice
      setDirection(index > mobileIndex ? 1 : -1);
      setMobileIndex(index);
    },
    [emblaApi, mobileIndex]
  );
