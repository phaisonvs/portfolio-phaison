...
export function ProjectsSwiper({ projects }: ProjectsSwiperProps) {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (width < 640) {
        // Mobile: exatamente 2 cards
        setSlidesPerView(2);
        setSpaceBetween(8);
      } else if (width < 768) {
        // Tablet pequeno: 2 cards
        setSlidesPerView(2);
        setSpaceBetween(12);
      } else if (width < 1024) {
        // Tablet: 3 cards
  ...
  return (
    <div className="w-full">
      ...
      {/* Swiper Carousel - Ajustado para respeitar os limites do container */}
      <div className="w-full overflow-hidden projects-carousel-container">
        <Swiper
