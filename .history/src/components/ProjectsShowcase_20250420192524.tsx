...
export function ProjectsShowcase({ projects }: ProjectsShowcaseProps) {
  ...
  // Setup do carrossel Embla com plugins para melhor experiência
  const [emblaRef, emblaApi] = useEmblaCarousel(
    ...
    [WheelGesturesPlugin(), AutoPlay({ stopOnInteraction: true, delay: 8000 })]
  );

  const [selectedIndex, setSelectedIndex] = useState(0);

  // Função avançar com animação suave
  const scrollPrev = useCallback(() => {
    if (!emblaApi || isPending) return;
  ...
  // Função retroceder com animação suave
  ...
  // Efeito para registrar listeners do Embla
  useEffect(() => {
    if (!emblaApi) return;
