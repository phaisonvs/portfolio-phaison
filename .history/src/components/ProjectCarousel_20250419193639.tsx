      ...
      >
        ...
        <CarouselContent className="md:-ml-4">
          {visibleProjects.map((project, index) => (
            <CarouselItem
              key={project.id}
              // Remove min-w, mantém flex-shrink-0 e ajusta padding
              className="flex-shrink-0 px-2 md:px-0 md:pl-4"
            >
              {/* h-full garante altura consistente entre cards na mesma linha */}
              <Card className="flex flex-col bg-gray-900/40 dark:bg-gray-900/40 border border-gray-700/30 dark:border-gray-700/30 rounded-xl backdrop-blur-sm shadow-lg hover:border-emerald-800/40 dark:hover:border-emerald-800/40 transition-all h-full overflow-hidden">
                {" "}
                {/* Adicionado overflow-hidden ao card */}
                <CardContent className="p-0 flex flex-col h-full">
                  {/* Imagem com LARGURA fixa e altura automática baseada no aspect ratio */}
                  <div className="relative w-[200px] flex-shrink-0 overflow-hidden rounded-t-xl">
                    {" "}
                    {/* Largura fixa, remove altura fixa */}
                    <img
                      src={project.image}
                      alt={project.title}
                      className="object-cover h-full" /* object-cover e h-full, remove w-full */
                    />
                  </div>
                  {/* Conteúdo textual com padding e flex-grow */}
