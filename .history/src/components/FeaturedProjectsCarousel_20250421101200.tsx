...
export function FeaturedProjectsCarousel({
  ...
  useEffect(() => {
    function handleResize() {
      ...
      // Certifica-se de que o índice atual seja válido para o novo número de cards visíveis
  ...
  const bind = useDrag(
    ({ movement: [mx], velocity: [vx], direction: [dx], last }) => {
      ...
      if (last) {
        const threshold = width * 0.15; // 15% da largura da tela
        ...
        const targetX = currentOffset + (mx / width) * slideWidth * 2;
  ...
  // Calcula a largura percentual ideal para cada card
  const getCardWidth = () => {
    const gapPercent = 2; // Margem entre os cards (%)
    const availableWidthPercent = 100 - gapPercent * (visibleCards - 1);
    return availableWidthPercent / visibleCards;
  };
  ...
  return (
    <div className="space-y-8 w-full overflow-hidden">
      ...
      <div
        className="w-full px-0 sm:px-2 lg:px-0 max-w-full"
        role="region"
              ...
              {projects.map((project, index) => (
                <ReactSpring.animated.div
                  ...
                  style={{
                    ...
                    paddingRight: index === projects.length - 1 ? 0 : "1%",
