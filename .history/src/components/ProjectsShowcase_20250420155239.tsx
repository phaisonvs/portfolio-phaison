        ...
        <div className="flex items-center justify-between mt-8 gap-3">
          <div className="flex-1 flex items-center">
            <div className="hidden sm:flex justify-start gap-2">
              {Array.from({
                length: Math.ceil(projects.length / visibleItems),
              }).map((_, index) => (
                <button
                  key={index}
                  ...
                  onClick={() => {
                    setDirection(
                      index > Math.floor(currentIndex / visibleItems) ? 1 : -1
                    );
                    setCurrentIndex(index * visibleItems);
                  }}
                  aria-label={`Ir para grupo de projetos ${index + 1}`}
                />
              ))}
            </div>
          </div>
          <div className="flex gap-3">
            <motion.button
              onClick={prevProject}
              className="bg-gray-800/70 backdrop-blur-sm p-2.5 rounded-full text-white hover:bg-gray-700 transition-colors border border-gray-700/30"
                ...
                <button
                  ...
                  className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
                    index === currentIndex
                      ...
                      : "bg-gray-600 hover:bg-gray-500 scale-100 opacity-70"
                  }`}
                  onClick={() => {
